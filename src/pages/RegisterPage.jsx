...
/* ---- Toggle password reusable di dalam page ini ---- */
...
function RegisterPage() {
  ...
  const validate = () => {
    ...
    if (!form.name.trim()) {
      e.name = 'Nama lengkap tidak boleh kosong.'; valid = false
    }
    if (!form.email || !/\S+@\S+\.\S+/.test(form.email)) {
      e.email = 'Masukkan alamat email yang valid.'; valid = false
    }
  ...
  const handleSubmit = (e) => {
    e.preventDefault()
    if (validate()) {
      // TODO: hubungkan ke endpoint backend register
      navigate('/login')
    }
  }
